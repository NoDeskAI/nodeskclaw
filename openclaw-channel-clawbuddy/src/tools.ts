...
function resolveToolConfig(config: OpenClawConfig): ToolConfig {
  ...
  return {
    apiUrl: process.env.CLAWBUDDY_API_URL || "http://localhost:8000/api/v1",
    token: account.apiToken || process.env.CLAWBUDDY_TOKEN || "",
    workspaceId: account.workspaceId || process.env.CLAWBUDDY_WORKSPACE_ID || "",
...
export function createClawBuddyTools(config: OpenClawConfig): AnyAgentTool[] {
