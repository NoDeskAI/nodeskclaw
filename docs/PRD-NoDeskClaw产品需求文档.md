...
- 普通成员通过 7 项独立权限控制：管理设置、管理 Agent、成员管理、编辑黑板、发送聊天、编辑拓扑、删除办公室
...
## 附录 B 赛博办公室 Agent 协同功能

### B.1 功能概述

赛博办公室内的多个 Agent 以"群聊"模式协同工作。用户发送消息后，所有办公室内运行中的 Agent 同时收到消息并各自决定是否回复。Agent 也可通过 `send` 工具主动联系其他 Agent。

### B.2 核心交互

- **群聊广播**：用户消息自动发送给办公室内所有运行中 Agent
...
- **入群自我介绍**：Agent 加入办公室后自动收到系统消息，触发自我介绍回复
...
添加 Agent 到办公室时展示步骤进度条（替代原按钮 spinner），4 个阶段按时间推进：
...
添加成功后弹出 Toast 通知，包含"前往查看"按钮，点击后跳转到办公室并将 3D 摄像机平滑聚焦到新 Agent 的工位。Agent 实例重启完成后，后端通过 `agent:status` SSE 事件实时推送状态变更，前端自动更新状态徽标（无需手动刷新）。

### B.2.3 Agent 状态颜色

办公室视图中 Agent hex 使用橙色 `#f97316` 标识过渡状态（restarting、deploying、updating、creating），与运行中（绿色）、错误（红色）明确区分。适用于 2D 视图、3D 视图和 MiniHexPreview
...
**维度 3 - 社交进化**：办公室不拥有基因。Agent 基于团队目标自主判断需要哪些基因，通过群聊消息推荐给其他 Agent。
...
将人类成员作为办公室网格中的通讯节点，与 Agent Hex 地位对等。
...
- **飞书私聊（自动）**：用户通过飞书 SSO 登录后，系统自动获取 `open_id`，Agent 消息以卡片形式直接发送到用户飞书私聊，无需额外配置
- **飞书群聊（手动配置）**：Human Hex 的 Channel 配置中填入群聊 `chat_id`，Agent 消息推送到指定群聊。适用于团队协作场景
- **SSE 回退**：未登录飞书且未配置群聊时，消息通过 workspace SSE 事件推送到前端 Web 界面

**回复路由**：人类在飞书中直接回复消息，系统通过 `open_id`（私聊）或 `chat_id`（群聊）匹配到对应 Human Hex，再经拓扑路由引擎投递到可达的 Agent
...
- 办公室定时触发器（workspace_schedules）：平台提供"墙上的钟"，通过 cron 表达式定时向 Agent 发送系统消息（如站会、周报）
