...
**NoDeskClaw** 是一个 K8s 可视化管理控制台，通过简洁直观的 Web 界面，让公司成员无需 K8s 操作经验，即可一键部署和管理 OpenClaw（AI 员工平台）。
...
OpenClaw 是一个强大的 AI 员工平台，但部署和运维需要一定的 K8s 技术门槛，包括编写 YAML、执行 kubectl 命令、排查 Pod 异常等。为了让公司所有成员都能便捷使用，需要提供可视化的部署和管理界面。
...
| 扩缩容 | 快捷调整副本数 |
| 删除 | 删除整个 OpenClaw 实例（二次确认）。若实例已加入办公室，必须先从办公室移除才能删除 |
...
赛博办公室采用"预设角色 + checkbox 微调"模型：

- 创建者自动成为管理员（`is_admin=True`），拥有全部权限
- 普通成员通过 7 项独立权限控制：管理设置、管理 AI 员工、成员管理、编辑黑板、发送聊天、编辑拓扑、删除办公室
- 添加成员时可选预设角色（管理员/协作者/观察者）快速填充权限，再按需微调
- 组织管理员隐式拥有所有办公室的全部权限
- 办公室列表仅显示用户为成员的办公室（组织管理员可见全部）
- 移除成员时自动清理其人类工位并发送通知
...
toast.success('Agent 已添加到办公室', { action: { label: '前往查看', onClick: () => router.push(...) } })
```
...
## 附录 B 赛博办公室 AI 员工协同功能

### B.1 功能概述

赛博办公室内的多个 AI 员工以"群聊"模式协同工作。用户发送消息后，所有办公室内运行中的 AI 员工同时收到消息并各自决定是否回复。AI 员工也可通过 `send` 工具主动联系其他 AI 员工。

### B.2 核心交互

- **群聊广播**：用户消息自动发送给办公室内所有运行中 AI 员工
- **@ 提及**：用户可 `@AgentName` 定向标记 AI 员工。被 @ 的 AI 员工收到"必须回复"提示，未被 @ 的 AI 员工收到"如无关可 NO_REPLY"提示。消息仍然全员广播
- **/ 命令**：输入 `/` 触发命令菜单。Phase 1 支持 `/status`（查看状态）、`/clear`（清空聊天）、`/restart AgentName`（重启 AI 员工）、`/remove AgentName`（移除 AI 员工）。命令在前端本地执行，不发送给 AI 员工
- **多 AI 员工流式响应**：各 AI 员工响应通过 SSE 实时推送到前端
- **NO_REPLY 静默**：与话题无关的 AI 员工回复 `NO_REPLY` 即可静默，后端仍广播 `agent:done` 清除 typing 指示器，前端无消息显示
- **AI 员工协同**：AI 员工使用 `send -t nodeskclaw -to "agent:xxx" -m "消息"` 主动联系其他 AI 员工
- **防循环**：AI 员工响应只入库不触发其他 AI 员工；主动 send 携带深度计数器
- **入群自我介绍**：AI 员工加入办公室后自动收到系统消息，触发自我介绍回复
...
- 系统消息（如"XXX 已加入办公室"）以居中灰色气泡展示，与 user/agent 消息区分
- 用户头像：有 `avatar_url` 时显示真实头像图片，无则回退为灰色圆圈 + User 图标
- @ 提及 Tag 输入：在任意位置输入 `@` 即可弹出 AI 员工列表浮动下拉（无需前置空格，适配中文无词间空格的输入习惯），选中后以 Tag 形式内嵌在编辑器中（Bot 图标 + 名称，hover 时图标变为 x 删除按钮，hover 弹出 Popover 展示 AI 员工状态/slug），Backspace 原子删除整个 Tag；消息中的 `@AgentName` 高亮渲染（用户气泡内白色半透明底、AI 员工气泡内主色调底）
- / 命令 Tag 输入：输入 `/` 后弹出命令列表浮动下拉（每行末尾标注"立即执行"或"Tag"类型），支持键盘导航；立即执行型命令（`/clear`、`/status`）选中后直接执行；needsAgent 命令（如 `/remove`）选中后自动触发链式 AI 员工选择，合并为单一 `/remove @AgentName` Tag
...
- AI 员工消息名称/slug 显示：名称最大 120px、slug 最大 140px，溢出省略（ellipsis）+原生 title tooltip，hover 时右侧出现复制按钮复制 slug（toast 提示"已复制"）
- AI 员工消息气泡宽度 92%（用户消息保持 75%），AI 员工消息内容支持 Markdown 渲染（使用 `marked` 库，GFM + breaks 模式，支持粗体、列表、代码块、引用等，配套 `.chat-markdown` prose 样式）
- 斜杠命令结果（`/status` 等）持久化到后端，刷新后仍可见；`/clear` 仅清空前端显示

### B.2.2 添加 AI 员工进度与通知

添加 AI 员工到办公室时展示步骤进度条（替代原按钮 spinner），4 个阶段按时间推进：
...
添加成功后弹出 Toast 通知，包含"前往查看"按钮，点击后跳转到办公室并将 3D 摄像机平滑聚焦到新 AI 员工的工位。AI 员工实例重启完成后，后端通过 `agent:status` SSE 事件实时推送状态变更，前端自动更新状态徽标（无需手动刷新）。

### B.2.3 AI 员工状态颜色

办公室视图中 AI 员工 hex 使用橙色 `#f97316` 标识过渡状态（restarting、deploying、updating、creating），与运行中（绿色）、错误（红色）明确区分。适用于 2D 视图、3D 视图和 MiniHexPreview

### B.3 技术实现

- 后端维护 `workspace_messages` 表，记录所有群聊消息
- 每次调用 AI 员工时注入其他成员的近期消息作为上下文
- OpenClaw session key `workspace:{ws_id}` 隔离办公室会话
- 自研 `openclaw-channel-nodeskclaw` channel plugin 实现 AI 员工间通信
...
办公室画布（3D / 2D）采用六边形（Hex）网格布局，所有 Hex 均可点击，点击后弹出底部操作抽屉（HexActionDrawer），抽屉内容根据 Hex 类型动态切换：

| Hex 类型 | 抽屉操作 |
|----------|---------|
| 空位 | 添加 AI 员工到此工位、放置过道、放置人类工位 |
| AI 员工 | 打开对话、查看详情、移动、从办公室移除 |
| 过道 | 重命名、管理连接方向、移动、删除 |
...
- **双击 AI 员工 Hex** → 直接打开群聊侧边栏（快捷方式）
- **方向键 / WASD** → 始终平移画布（不再用于移动 Hex）
- **移动 Hex** → 通过操作抽屉中的「移动」按钮进入移动模式，画布顶部显示提示条和取消按钮，空格高亮为可选目标，点击空格完成移动。支持 AI 员工 / 过道 / Human 三种 Hex 类型。Escape 或点击取消按钮退出移动模式

### C.3 添加 AI 员工到指定工位

从画布空位点击「添加 AI 员工到此工位」时，将 `hex_q` / `hex_r` 作为 query 参数传递到 AddAgent 页面。后端 `AddAgentRequest` 支持可选的 `hex_q` / `hex_r`，有则使用指定坐标，无则使用螺旋布局自动分配。
...
基因进化生态系统让 AI 员工不只是被动接收能力，还能学习、成长、创造、分享基因，形成真正的进化闭环。
...
**维度 1 - 学习内化**：AI 员工通过内置 `meta-learning` 基因自主决定获取方式（直接安装 vs 深度学习）。学习通过 Learning Channel Plugin 异步进行。

**维度 2 - 基因回馈**：AI 员工个性化学习产物经实战验证后回流市场。使用次数达门槛 -> AI 员工自主申请发布 -> 人类审核 -> 变体上架。

**维度 3 - 社交进化**：办公室不拥有基因。AI 员工基于团队目标自主判断需要哪些基因，通过群聊消息推荐给其他 AI 员工。

**维度 4 - 基因协同**：三种来源（发布者标注 + 系统共装分析 + AI 员工主动发现），详情页展示推荐搭配。

**维度 5 - 自然选择**：效能分 = 用户评分 25% + AI 员工自评 25% + 使用效能 50%。群聊中赞/踩按钮提供轻量反馈。

**维度 X - AI 员工创造基因**：AI 员工自主触发或用户指导，产出完整基因包，两步审核（实例所有者 -> Admin）。

### D.4 基因市场

- 视图切换：基因 / 基因组 / AI 员工模板 / 进化趋势
- 搜索栏 + 标签筛选 + 领域分类
...
数据源为 Pod 实际 skills 扫描（`GET /instances/:id/skills`），而非纯 DB 记录，确保与实例运行时状态一致。

**基因分类展示**：

- **Hub 基因**：来源于基因 Hub 的 skill，slug 可在 Hub 中找到匹配。展示完整基因详情（名称、标签、状态、效能分、版本、使用次数），支持发布变体、遗忘等操作。
- **涌现基因**：通过 Meta Learning 或其他机制涌现的 skill，在 Hub 中找不到匹配。标记为"基因涌现"，展示 FrontMatter 中的 name/description、文件数量、常驻激活标记。点击卡片可查看完整 SKILL.md 内容（Markdown 渲染）。

**DB 僵尸记录清理**：扫描 Pod skills 时，对 DB 中 status 为终态（installed/failed/simplified 等）但 Pod 上已不存在对应 skill 的 InstanceGene 记录，自动执行软删除，释放唯一约束以允许重新安装。过渡态记录（learning/installing 等）不受影响。

- 支持 `focus_gene_id` 落地参数，进入实例基因页后将目标基因置顶展示，便于继续操作

**内嵌基因学习 Dialog**：点击"学习基因"按钮打开 Dialog（不离开当前页面），Dialog 内嵌完整基因市场浏览功能（基因/基因组 Tab、搜索筛选、卡片网格、详情查看），学习操作自动绑定当前实例，无需再次选择实例。已学习基因在卡片上显示角标（绿色勾）。

**过渡状态自动刷新**：当存在处于 `learning`/`installing`/`forgetting`/`uninstalling` 过渡状态的基因时，页面每 8 秒自动轮询刷新列表（Pod 扫描比纯 DB 查询稍慢），所有基因进入稳定状态后自动停止轮询。轮询静默更新数据，不展示全屏 loading。

**Pod 不可用降级**：当实例 Pod 未运行时，页面展示"实例不可用，无法加载基因列表"错误提示，不回退到 DB 历史数据。

**遗忘仪式**：仅在实例基因管理页执行重确认流程，防止误操作。执行遗忘前展示该基因影响的技能预览（依赖该基因的 skill 列表），用户需输入基因名称进行二次确认后方可完成遗忘。

**深度遗忘**：AI 员工回顾该基因的使用经验（调用记录、上下文关联），用户可选择完全遗忘（彻底移除）或简化保留（保留精简版能力描述，移除完整实现）。

**进化日志**：记录 AI 员工所有基因事件的时间线，包括学习、遗忘、简化、发布变体、应用基因组等操作，支持按时间筛选和事件类型过滤。

### D.5.1 AI 员工模板

AI 员工模板存储一组基因 slug 列表，用于快速创建具备预设基因组合的新 AI 员工实例。

**创建路径**:
- 从基因市场手动选择基因组合创建模板
- 从已运行的 AI 员工"保存为模板"（自动提取已安装基因列表）

**使用路径**:
- 在创建 AI 员工页面选择模板，部署完成后自动安装模板中的所有基因
- 从模板详情页点击"使用此模板"直接跳转创建页面

**模板管理**:
- 基因市场新增"AI 员工模板"Tab，展示模板卡片（名称、描述、基因数量、使用次数）
- 模板详情页展示包含的基因列表、使用统计，支持删除
- 实例基因页操作栏新增"保存为模板"按钮

**部署集成**:
- `DeployRequest` 新增 `template_id` 字段
- 部署管道完成后逐个调用 `install_gene()` 安装模板基因
- 单个基因安装失败不阻断整个部署，仅记录 warning
- 安装完成后模板 `use_count` +1
...
- 页面级约束：P03 实例列表、办公室列表页、成员管理页（含表格表头、状态文案、按钮、搜索占位符、计数文案）统一走 i18n 词条
...
办公室六边形网格引入过道机制，实现有向信息路由，替代当前全局广播模式。

- 四种 Hex 实体：黑板 Hex（中心枢纽）、AI 员工 Hex（AI 工作节点）、Human Hex（人类通讯节点）、过道 Hex（中继节点）
- 两种连接机制：手动直连（两个相邻 Hex 间创建有向连接）、过道 Hex 自动打通（放置时自动与相邻 Hex 建立双向连接）
- 有向控制：每条连接支持 both/a_to_b/b_to_a 三种方向
- 路由引擎：BFS 有向图遍历，过道 Hex 透传 + AI 员工/Human Hex 终止
- 向后兼容：无连接/过道时保持全局广播行为

### D.9 Human Hex（人类节点化）

将人类成员作为办公室网格中的通讯节点，与 AI 员工 Hex 地位对等。

- 架构对称性：AI 员工 Hex 背后是 OpenClaw 实例（LLM 处理），Human Hex 背后是 Channel（飞书群/机器人，转发给人类）
- 人类视为永远在线（消息到达 Human Hex 后通过 Channel 适配器推送到飞书群）
- 人类在飞书回复后通过 webhook 注入回办公室，路由引擎继续投递
- 3D 渲染：更细更高的暖色六棱柱，发光边缘始终亮起
- 数据模型：复用 workspace_members 表，扩展 hex_q/r、channel_type、channel_config、display_color 字段

**消息投递方式**：

- **飞书私聊（自动）**：用户通过飞书 SSO 登录后，系统自动获取 `open_id`，AI 员工消息以卡片形式直接发送到用户飞书私聊，无需额外配置
- **飞书群聊（手动配置）**：Human Hex 的 Channel 配置中填入群聊 `chat_id`，AI 员工消息推送到指定群聊。适用于团队协作场景
- **SSE 回退**：未登录飞书且未配置群聊时，消息通过 workspace SSE 事件推送到前端 Web 界面

**回复路由**：人类在飞书中直接回复消息，系统通过 `open_id`（私聊）或 `chat_id`（群聊）匹配到对应 Human Hex，再经拓扑路由引擎投递到可达的 AI 员工
...
- 成员状态（member_status）：实时显示 AI 员工和 Human 的当前状态和正在进行的任务
- 绩效指标（performance）：每个成员的多维度绩效数据
- 办公室定时触发器（workspace_schedules）：平台提供"墙上的钟"，通过 cron 表达式定时向 AI 员工发送系统消息（如站会、周报）
- 拓扑可观测性：消息流量热力图、消息追踪、拓扑健康度检测、审计日志
- 决策审计链：记录 AI 员工重要决策的完整上下文（提议、依据、审批、结果），作为自我改进基础
...
- AI 员工需要授权时通过过道向 Human Hex 发送结构化审批请求
...
- 版本管理：AI 员工通过 MCP 工具管理产物版本

### D.14 元基因（组织自治能力）

赋予 AI 员工组织级自我管理能力的特殊基因/基因组。

- AI-HC 基因（自主招聘）：分析能力缺口，起草 HC 申请单，通过信任机制审批后部署新 AI 员工
...
- 从模板创建：选择模板一键部署完整办公室
- 模板市场：在基因市场中新增"办公室模板"分类

### D.17 企业空间（文件浏览）

组织管理员在 Portal 中以只读方式浏览所有 AI 员工实例内的文件，解决文件散落在不同 AI 员工之间难以管理的问题。

- **入口**：Portal 顶部导航"企业空间"（仅组织 admin 可见）
- **AI 员工列表**：展示组织下所有实例，标注运行状态；运行中的可进入文件浏览，非运行的灰显提示
