...
| 扩缩容 | 快捷调整副本数 |
| 删除 | 删除整个 OpenClaw 实例（二次确认）。若实例已加入工作区，必须先从工作区移除才能删除 |
...
工作区采用"预设角色 + checkbox 微调"模型：

- 创建者自动成为管理员（`is_admin=True`），拥有全部权限
- 普通成员通过 7 项独立权限控制：管理设置、管理 Agent、成员管理、编辑黑板、发送聊天、编辑拓扑、删除工作区
- 添加成员时可选预设角色（管理员/协作者/观察者）快速填充权限，再按需微调
- 组织管理员隐式拥有所有工作区的全部权限
- 工作区列表仅显示用户为成员的工作区（组织管理员可见全部）
- 移除成员时自动清理其人类工位并发送通知
