...
**内嵌基因学习 Dialog**：点击"学习基因"按钮打开 Dialog（不离开当前页面），Dialog 内嵌完整基因市场浏览功能（基因/基因组 Tab、搜索筛选、卡片网格、详情查看），学习操作自动绑定当前实例，无需再次选择实例。已学习基因在卡片上显示"已学习"徽标并在详情页禁用学习按钮。

**过渡状态自动刷新**：当存在处于 `learning`/`installing`/`forgetting`/`uninstalling` 过渡状态的基因时，页面每 5 秒自动轮询刷新基因列表，所有基因进入稳定状态后自动停止轮询。轮询静默更新数据，不展示全屏 loading。
