...
- 覆盖范围：`claw-buddy-portal`（用户门户前端） + `claw-buddy-frontend`（管理前端） + `claw-buddy-backend`（后端 API 错误契约）
...
## E. 过道系统与 Human Hex（Corridor System & Human Hex）

### E.1 概述

工作区从"全局广播"升级为"拓扑化通信"。通过过道（Corridor）机制，实现消息的定向传递、隔离和分层管理。同时引入 Human Hex，让人类以通讯节点的形式参与工作区拓扑。

### E.2 四种 Hex 实体

| 实体 | 定义 | 消息行为 |
|------|------|----------|
| 黑板 Hex | 固定在 (0,0)，存储结构化数据 | 系统消息/人类指令从此出发 |
| Agent Hex | AI 工作节点，背后是 OpenClaw 实例 | 消息终点，LLM 处理 |
| Human Hex | 人类通讯节点，背后是 Channel（飞书群） | 消息终点，转发到飞书，人类在飞书回复 |
| 过道 Hex | 中继节点，类似网络交换机 | 透传，自动转发到所有另一侧连接 |

### E.3 两种连接机制

- **直连**：手动在两个相邻已占用 Hex 间创建有向连接
- **过道 Hex 自动打通**：放置过道 Hex 时，自动与所有相邻已占用 Hex 建立双向连接

### E.4 Human Hex 通讯节点模型

Human Hex 不是"人类坐在电脑前看工作区"，而是一个通讯节点：

- 消息到达 Human Hex → Channel 适配器调用飞书 API → 飞书群收到消息
- 人类在飞书群回复 → Channel Webhook 接收 → 注入回工作区 → 路由引擎投递
- 人类视为永远在线（飞书能推送，不依赖打开网页）
- 3D 渲染：更细更高的六棱柱，默认暖色（琥珀），可自定义

### E.5 消息路由规则

- 路由引擎使用 BFS 有向图遍历
- Agent/Human Hex 是终点，过道 Hex 是中继
- 每条连接有方向控制（双向/A→B/B→A）
- 无任何连接时，保持现有全局广播行为（向后兼容）

---

## F. 黑板 v2（Blackboard v2）

### F.1 概述

黑板从纯文本（auto_summary + manual_notes）升级为结构化管理面板，承载任务管理、目标追踪、成员状态、绩效考核。

### F.2 结构化字段

| 字段 | 类型 | 说明 |
|------|------|------|
| objectives | JSON | OKR 目标，含 key_results 和进度 |
| tasks | JSON | 任务看板（todo/doing/done/blocked），含指派、优先级、deadline |
| member_status | JSON | 成员状态面板（在线/忙碌/离开等），系统自动维护 |
| performance | JSON | 绩效指标（完成率、协作效率等） |
| manual_notes | Text | 保留原有手动备注 |
| auto_summary | Text | 增强为基于拓扑的智能摘要 |

### F.3 工作区定时触发器

平台提供"墙上的钟"，基因决定"看钟做什么"：
- 支持 cron 表达式定义触发频率
- 触发时向黑板连接的所有 Agent 发送系统消息
- 预设模板：站会（每日）、周报（每周）、冲刺回顾（每两周）

### F.4 决策审计链

记录每个 Agent 做出的重要决策的完整上下文：
- 谁提议、基于什么信息、谁审批、执行结果
- Agent 自我改进循环的原材料
- 人类可查看决策链，理解 AI 公司运转逻辑

---

## G. MCP 集成（Model Context Protocol）

### G.1 概述

为 Agent 接入外部工具能力。支持实例级 MCP Server 配置，以及通过基因 manifest 自动注入。

### G.2 实例级 MCP 配置

- 用户在实例设置中添加 MCP Server（stdio/SSE 两种传输）
- 后端通过 NFS 写入 openclaw.json 的 MCP 配置
- 支持手动添加和通过基因自动注入两种方式

### G.3 ClawBuddy Self-MCP 基因组

让 Agent 能操作 ClawBuddy 自身的一组 MCP 工具，以基因/基因组形式发布到基因市场：

| 基因 | 能力 |
|------|------|
| clawbuddy-blackboard-tools | 读取/更新任务、目标、产出 |
| clawbuddy-topology-awareness | 查询邻居、成员、连接状态 |
| clawbuddy-performance-reader | 读取绩效数据 |
| clawbuddy-proposals | 提交/读取申请单（HC、重组、创新） |
| clawbuddy-gene-discovery | 搜索基因市场、查询详情 |
...
授权流程由基因驱动：
- Agent 需要授权时，通过过道向 Human Hex 发送结构化审批请求
- 人类在飞书收到后选择 Allow this time / Allow always / Deny
- Allow always 存入信任策略表，后续自动放行
- 信任随时间积累，系统从"事事请示"演进到"高度自治"

### H.2 元基因

| 元基因 | 能力 |
|--------|------|
| AI-HC | 自主招聘：分析缺口、起草 HC 申请、部署新 Agent |
| 自主重组 | 分析低效拓扑、提交重组申请 |
| 文化基因 | 定义团队沟通风格和决策偏好 |
| 创新基因 | 回顾工作、识别改进机会、起草提案 |
...
工作流不是独立引擎，而是通过基因 + 过道 + 黑板的组合实现。每个工作流步骤是一个基因，定义上游、任务、完成标准、下游。

### I.2 工作流基因组

预制基因组一键安装一套工作流，可附带推荐的过道拓扑模板。
...
- 任务完成率、消息质量、协作效率、基因使用效果

### J.2 自我改进循环

Agent 通过"自我改进"基因：读取绩效 → 分析短板 → 搜索基因市场 → 起草学习计划 → 审批后自主学习。
...
调好的工作区拓扑+基因+黑板目标可一键保存为模板，快速复制。模板可在基因市场分享。

### K.2 模板内容

- 拓扑快照（Agent/Human/过道 Hex 位置 + 连接）
- 黑板模板（预设目标和任务分类）
- 基因推荐（每个位置的推荐基因/基因组）
